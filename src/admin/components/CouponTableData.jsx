...
const CouponTableData = ({
  ...
  const handleEditClick = (coupon) => {
    navigate(`/admin/dashboard/coupon/${coupon._id}`, { state: { coupon } });
  };
  return (
    <section className="flex bg-white py-4 drop-shadow-md rounded">
      ...
      <div className="flex-1 flex items-center justify-center">
        <span
          className={`capitalize text-green-700 ${
            coupon.status === "inactive" ? "text-red-600" : "text-green-600"
          }`}
        >
          {coupon.status}
        </span>
      ...
      <div className="flex-1 flex items-center justify-evenly gap-4">
        ...
        {coupon.status === "inactive" ? (
          ...
          <button
            onClick={() => handleChangeStatus(coupon._id, "inactive")}
